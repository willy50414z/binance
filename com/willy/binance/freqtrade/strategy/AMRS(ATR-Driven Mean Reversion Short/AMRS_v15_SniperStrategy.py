...
class AMRS_v15_SniperStrategy(IStrategy):
    """
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float,
        ...
        # 2. 持倉保護期：前 60 分鐘不輕易被指標洗掉
        trade_duration = (current_time - trade.open_date_utc).total_seconds() / 60
        if trade_duration < 60 and current_profit < 0.005:
            return None

        # 3. ATR 動態止盈：如果利潤超過 3 倍 ATR，代表超漲超跌，直接入袋
        # 假設 dataframe 已經計算了 'atr'
        atr_value = last_candle['atr']
        if current_profit > (atr_value / current_rate) * 3:
            return "v13_atr_target_hit"

        # 4. 嚴格摩擦成本保護
        if current_profit < 0.003 and trade_duration < 300:  # 5小時內不到0.3%不走
            return None
