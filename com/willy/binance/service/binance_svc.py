...
class BinanceSvc:
    ...
    def get_futures_historical_klines(self, binance_product: BinanceProduct, kline_interval=Client.KLINE_INTERVAL_1DAY,
                                      start_date: datetime = type_util.str_to_date("20250101"),
                                      end_date: datetime = type_util.str_to_date("20250105")) -> List[BinanceKline]:
        klines = self.client.futures_historical_klines(binance_product.name, kline_interval,
    ...
    def get_close_ma(self, binance_product: BinanceProduct, kline_interval=Client.KLINE_INTERVAL_1DAY,
        ...
        klines = self.get_futures_historical_klines(binance_product, kline_interval, start_date, end_date)
    ...
    def get_futures_open_orders(self, symbol: Optional[str] = None) -> List[CommissionOrder]:
        """
        取得當前委託單
        參數:
            symbol: 指定交易對（可選）
    ...
    def get_futures_symbol_ticker(self, binance_product: BinanceProduct):
        return self.client.futures_symbol_ticker(symbol=binance_product.name)

    def get_futures_klines(self, binance_product: BinanceProduct, kline_interval=Client.KLINE_INTERVAL_1DAY,
                           start_time: datetime = type_util.str_to_date("20250101"),
                           end_time: datetime = type_util.str_to_date("20250105")):
        klines = self.client.futures_klines(symbol=binance_product.name, interval=kline_interval,
    ...
    def create_test_spot_order(self, binance_product: BinanceProduct, trade_type: TradeType, order_type: OrderType,
        ...
        try:
            if order_type.binance_type == ORDER_TYPE_MARKET:  # 市價單
                result = self.client.create_test_order(
                    symbol=binance_product.name,
                    side=trade_type.bianace_type,
                    type=order_type.binance_type,
                    quantity=unit
                )
            elif order_type.binance_type == ORDER_TYPE_LIMIT or order_type.binance_type == ORDER_TYPE_STOP_LOSS:  # 限價單
    ...
    def create_futures_order(self, binance_product: BinanceProduct, trade_type: TradeType, order_type: OrderType,
                             unit: Decimal, price: str = None):
        req_msg = f"product[{binance_product}]trade_type[{trade_type.name}]order_type[{order_type.name}]qty[{unit}]"
        try:
            if order_type.binance_type == ORDER_TYPE_MARKET:  # 市價單
...
#     result = service.create_futures_order(BinanceProduct.BTCUSDT, TradeType.BUY, order_type=OrderType.LIMIT,
...
#     #
#     # 取得所有委託單
#     orders = service.get_futures_open_orders()
#     opened_order_log = ""
#     for order in orders:
